...
class Immediate extends Component {
    ...
    onScheduled=()=>{
            ...
            else if (pickup_point === '' && pp_street === '' && pickup_time === '' && receiver_name === '' && receiver_phone === '') {
                swal ( "Warning" ,  "Mandatory fields are missing" ,  "warning" );
                return false;
            }
            const totalCost = Array.isArray(parcels) && parcels.length > 0 ? parcels.reduce((accumulator, currentValue) => accumulator + currentValue.estimated_cost, 0):0;
...
export default withRouter(Immediate);